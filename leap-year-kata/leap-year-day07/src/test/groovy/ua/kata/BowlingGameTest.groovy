...
class BowlingGameTest extends Specification {
  def 'year is leap when is divisible by 4'() {
    given: 'a year that is divisible by 4'
    def year = new Year(4 * 100 + 4)

    expect: 'the year is a leap year'
    year.isLeap()
  }
  ...
  def 'year is not leap when is not divisible by 4'() {
    ...
    expect: 'the year is not a leap year'
    year.isNotLeap()
  }

  def 'year is not leap when is divisible by 100'() {
    given: 'a year that is divisible by 100'
