...
public class NestedJoins<K extends Comparable<K>, V> {
  ...
  private Flux<Triple<K, V, V>> nestedJoin(
    ...
    return primary.flatMap(
        primaryPair -> secondary.filter(byKeyEquality(primaryPair))
            .defaultIfEmpty(withNullifiedValue(primaryPair))
            .map(composeToTriple(composeRule, primaryPair))
    );
  }
  ...
  private Function<Pair<K, V>, Triple<K, V, V>> composeToTriple(
      BiFunction<Pair<K, V>, Pair<K, V>, Triple<K, V, V>> composeRule,
      Pair<K, V> primaryPair) {
    return secondaryPair -> composeRule.apply(primaryPair, secondaryPair);
  }
}
