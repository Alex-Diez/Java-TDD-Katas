...
public class NestedJoins<K extends Comparable<K>, V> {
  ...
  private Flux<Triple<K, V, V>> nestedJoin(
      ...
      BiFunction<Pair<K, V>, Pair<K, V>, Triple<K, V, V>> compositionOrder) {
    return primary.flatMap(
        primaryPair -> secondary.filter(byKeyEquality(primaryPair))
            .defaultIfEmpty(nullifiedValueFrom(primaryPair))
            .map(secondaryPair -> compositionOrder.apply(primaryPair, secondaryPair))
    );
  }
  ...
  private Pair<K, V> nullifiedValueFrom(Pair<K, V> primaryPair) {
