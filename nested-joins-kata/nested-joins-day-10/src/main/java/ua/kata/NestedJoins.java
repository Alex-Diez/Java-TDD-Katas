...
public class NestedJoins<K extends Comparable<K>, V> {
  ...
  private Flux<Triple<K, V, V>> nestedJoin(
      ...
      BiFunction<Pair<K, V>, Pair<K, V>, Triple<K, V, V>> compositionRule) {
    return primary.flatMap(
        primaryPair -> secondary.filter(byKeysEquality(primaryPair))
            .defaultIfEmpty(withNullifiedValue(primaryPair))
            .map(secondaryPair -> compositionRule.apply(primaryPair, secondaryPair))
    );
  }

  private Predicate<Pair<K, V>> byKeysEquality(Pair<K, V> primaryPair) {
