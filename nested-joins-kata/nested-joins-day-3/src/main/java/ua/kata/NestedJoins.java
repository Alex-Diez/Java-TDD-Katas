...
public class NestedJoins<K extends Comparable<K>, V> {
  ...
  private Flux<Triple<K, V, V>> nestedJoin(
    ...
    return primary.flatMap(
        primaryPair -> secondary.filter(secondaryPair -> primaryPair.getKey().compareTo(secondaryPair.getKey()) == 0)
            .switchIfEmpty(defaultPairWith(primaryPair.getKey()))
            .map(secondaryPair -> compose.apply(primaryPair, secondaryPair))
    );
  }

  private Flux<Pair<K, V>> defaultPairWith(K key) {
    return Flux.just(Pair.of(key, null));
  }
}
