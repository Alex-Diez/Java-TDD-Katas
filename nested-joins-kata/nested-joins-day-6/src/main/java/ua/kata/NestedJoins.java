...
public class NestedJoins<K extends Comparable<K>, V> {
  ...
  private Flux<Triple<K, V, V>> nestedJoin(
    ...
    return primary.flatMap(
        primaryPair -> secondary.filter(secondaryPair -> equalByKeys(primaryPair, secondaryPair))
            .defaultIfEmpty(nullifiedValue(primaryPair))
            .map(secondaryPair -> compose.apply(primaryPair, secondaryPair))
    );
  }

  private boolean equalByKeys(Pair<K, V> primaryPair, Pair<K, V> secondaryPair) {
    return primaryPair.getKey().compareTo(secondaryPair.getKey()) == 0;
  }

  private ImmutablePair<K, V> nullifiedValue(Pair<K, V> primaryPair) {
